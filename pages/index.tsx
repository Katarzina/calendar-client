          ...
          <div key={date}>
            ...
            {activities.map((activity, index) => {
              const mentor = mentorsData?.find(
                (mentorData: Mentor) => mentorData.id === activity.mentorId,
              );
              const userIdToCheck = userData?.id;
              const subscribeByActivity = subscriptions?.filter(
                (subscription) => subscription.activity.id === activity.id,
              );
              const containsUserId = subscribeByActivity.some(
                (subscription) => subscription.user.id === userIdToCheck,
              );
              return (
                <>
                  <div
                  ...
                  >
                    ...
                    <p>At: {moment(activity.startDate).format("HH-MM")}</p>
